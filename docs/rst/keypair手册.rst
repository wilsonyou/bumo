...
交易的执行有两种接口调用方式：\ `调用接口生成transaction\_blob <#transaction-blob>`__
和\ `自己生成 transaction\_blob <#id9>`__\ 。
...
2.填充protocol
buffer的交易对象Transaction，并进行序列化操作，从而得到transaction\_blob。具体的交易数据结构详情请看\ `ProtoBuf数据结构 <#protobuf>`__\ 。
